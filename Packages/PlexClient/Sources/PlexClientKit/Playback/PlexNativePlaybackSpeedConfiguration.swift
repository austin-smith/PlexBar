...
@MainActor
public enum PlexNativePlaybackSpeedConfiguration {
    public static let speeds = PlexPlaybackRate.allCases.map { playbackRate in
        AVPlaybackSpeed(
    ...
    #if os(tvOS)
    public static func apply(to playerViewController: AVPlayerViewController) {
        guard playerViewController.speeds.map(\.rate) != speeds.map(\.rate) else { return }
        playerViewController.speeds = speeds
    }
    #endif

    public static func speed(for playbackRate: PlexPlaybackRate) -> AVPlaybackSpeed? {
        speeds.first { speed in
            abs(speed.rate - playbackRate.rawValue) < 0.001
        }
    }

    public static func playbackRate(for speed: AVPlaybackSpeed?) -> PlexPlaybackRate? {
